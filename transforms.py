...
class FlattenJoinedDataFn(beam.DoFn):
    ...
    def process(self, element):
        try:
            ...
            logging.error(f"Error in FlattenJoinedDataFn: {e}")
            logging.error(f"Processing element {element} caused an error.")
            logging.error(traceback.format_exc())
            logging.warning("The error is not blocking since correct elements will continue to be processed")
...
class WriteToJsonFn(beam.DoFn):
    ...
    def process(self, element):
        try:
            ...
            logging.error(f"Error writing to {self.output_filename}: {e}")
            logging.error(traceback.format_exc())
            # Optionally, we could yield to an 'error' output for further analysis
            logging.warning("The error is not blocking since correct elements will continue to be processed")
...
class AggregateMetricsFn(beam.DoFn):
    def process(self, element):
        try:
            ...
            logging.error(f"Error in AggregateMetricsFn: {e}")
            logging.error(f"Processing element {element} caused an error.")
            logging.error(traceback.format_exc())
            logging.warning("The error is not blocking since correct elements will continue to be processed")
...
class AggregateAdvertiserMetricsFn(beam.DoFn):
    def process(self, element):
        try:
            ...
            logging.error(f"Error in AggregateAdvertiserMetricsFn: {e}")
            logging.error(f"Processing element {element} caused an error.")
            logging.error(traceback.format_exc())
            logging.warning("The error is not blocking since correct elements will continue to be processed")
...
class GetTopAdvertisersFn(beam.DoFn):
    def process(self, element):
        try:
            ...
            logging.error(f"Error in GetTopAdvertisersFn: {e}")
            logging.error(f"Processing element {element} caused an error.")
            logging.error(traceback.format_exc())
            logging.warning("The error is not blocking since correct elements will continue to be processed")
...
class AggregateUserSpendFn(beam.DoFn):
    def process(self, element):
        try:
            ...
            logging.error(f"Error in AggregateUserSpendFn: {e}")
            logging.error(f"Processing element {element} caused an error.")
            logging.error(traceback.format_exc())
            logging.warning("The error is not blocking since correct elements will continue to be processed")
...
class CalculateMedianSpendFn(beam.DoFn):
    def process(self, element):
        try:
            ...
            logging.error(f"Error in CalculateMedianSpendFn: {e}")
            logging.error(f"Processing element {element} caused an error.")
            logging.error(traceback.format_exc())
            logging.warning("The error is not blocking since correct elements will continue to be processed")